...
function CompanySidebar() {
  ...
  const nav = [
    ...
    { to: '/company/talent-search', label: 'Talent Search' },
    { to: '/company/jobs', label: 'Jobs' },
    { to: '/company/saved', label: 'Saved Candidates' },
    { to: '/company/shortlisted', label: 'Shortlisted Candidates' },

    { to: '/company/feedback', label: 'Interviews' },
    { to: '/company/communications', label: 'Communications' },
  ...
  return (
    <aside className="hidden w-64 shrink-0 lg:block">
      <div className="sticky top-[5.25rem] rounded-2xl border border-white/10 bg-white/5 p-4">
        <div className="text-xs font-semibold text-white/70 mb-3">Company</div>
...
export default function CompanyLayout() {
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] text-white">
      <div className="border-b border-white/10 bg-[#0B1020]/60 backdrop-blur supports-[backdrop-filter]:bg-[#0B1020]/50">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
          <Link to="/company" className="flex items-center gap-3">
            ...
            <div>
              <div className="text-sm font-bold">EPS Company</div>
              <div className="text-[10px] text-white/50">Hiring portal</div>
            </div>
          </Link>
      ...
      <div className="mx-auto grid max-w-6xl grid-cols-1 lg:grid-cols-[16rem_1fr] gap-6 px-4 py-6">
        <CompanySidebar />
        <div>
          <Outlet />
        </div>
