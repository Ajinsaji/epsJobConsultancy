...
export default function SavedCandidates() {
  ...
  const fetchSaved = async () => {
    setLoading(true)
    try {
      const res = await axios.get('/api/v1/companies/me/candidates/saved')
      setCandidates(res.data?.candidates || res.data || [])
    } catch (err) {
      toast.error(err?.response?.data?.message || 'Failed to load saved candidates')
      setCandidates([])
    } finally {
      setLoading(false)
    }
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Keep track of candidates you saved during talent search.
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center">
          <p className="text-sm font-semibold text-slate-700">No saved candidates yet.</p>
          <p className="mt-1 text-xs text-slate-500">Start exploring talent search.</p>
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {candidates.map((c) => (
